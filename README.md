...
### 1.7.3 (2026-07-11)
#### Added | 新增
- 新增一个小功能，现在能够记住你在问卷填了什么了，并当你再次遇到同一份问卷时提供一个“一键填写”功能
...
```
#defNS math
using namespace qlgstd; // 部分保留字是 qlgstd 下的函数
...
在 VSCode 中，直接在代码中输入 `#include <math>` 即可。扩展会自动补全库文件名。
...
| `short` | 16位有符号整数 | 0 |
| `long` | 32位有符号整数 | 0 |
| `longlong` / `int64` | 64位有符号整数 | 0 |
| `unsigned` / `uint` | 32位无符号整数 | 0 |
| `float` | 单精度浮点 | 0.0 |
| `double` | 双精度浮点 | 0.0 |
| `string` | 字符串 | "" |
| `char` | 字符 | '' |
...
如果你不想声明命名空间，可以声明命名空间为 `qlgstd`。

主代码（非库文件）**不允许**使用 `#defNS`。
...
支持的格式化符：`%d`（整数）、`%s`（字符串）、`%x`（十六进制）、`%X`（十六进制大写）、`%o`（八进制）、`%p`（指针）、`%f`（浮点数）
...
| `_gcd(a, b)` | 最大公约数 |
| `parseInt(str)` | 字符串转整数 |
| `_short(v)` | 截断为 16 位有符号整数 |
| `_int(v)` / `_int32(v)` | 截断为 32 位有符号整数 |
| `_long(v)` | 截断为 32 位有符号整数 |
| `_longlong(v)` / `_int64(v)` | 截断为 64 位有符号整数 |
| `_uint(v)` / `_unsigned(v)` | 零扩展为 32 位无符号整数 |
...
> **注意**：`qid->` 箭头访问方式在 v1.7.1 中已弃用，请改用 `qid.`。使用旧语法会触发警告。
...
- **不允许**使用 `#defNS` 指令
- 必须包含 `int main()` 或 `int main(int qid)` 入口函数（也可以从外部库加载 `qlgstd` 命名空间下的 `main` 函数）
- 必须 `return 0;`
...
2. 库文件不能包含 `main` 函数
3. 库文件后缀名为 `.qlg`
4. 库文件存放在工作目录的 `library/` 子目录下，或与主文件同一目录
