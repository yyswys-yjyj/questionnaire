  ...
  "tools": [
    {
      ...
      "parameters": [
        ...
        },
        {
          "name": "resultcode",
          "description": {
            "zh": "结果脚本(可选)，与result互斥。QLang v2：问卷XML内用<resultcode>子标签包裹代码。类C++弱缩进脚本，必须写int main()入口并return 0;。分号分隔，无需缩进。基础：int/bool/string/char类型、一维二维数组、for/while(嵌套break continue)、if-else(含and or not)、四则运算含取模、比较、字符串拼接、print(逗号多参)/cout输出、i++/--/+=、const只读变量、1e9、注释。问卷数据通过参数传入：int main(int qid)，通过指针访问 qid->q1_num(int) 和 qid->q1_text(string)。各题型映射：text/textarea→num=-1 text=原文(textarea换行符转\n)；single→num=1 text=选项文本；multiple→num=选择数 text=选项空格隔开；rating/likert/nps→num=按钮编号 text=编号；time→num=时间戳秒数 text=HH:MM:SS。内置函数：_gcd parseint。自定义函数。PHP变量：$var=value。STL容器(容量1000)：stack push/pop/top/size/empty。 queue push/pop/front/back/size。 vector push_back/pop_back/get/set/size/clear。 pair first/second。 priority_queue大顶堆 push/pop/top/size/empty。 cstring：sizeof size strlen strcmp strcpy。printf格式化(%d/%s/%x/%o/%p)。指针(&取地址/*解引用)。链式作用域(块/函数)。地址表空间256MB，超限报错。总时限10秒。结构体：struct定义，Node* p=new Node; p->val=1; p->next=0; while(p!=0) p=p->next。空指针报错。异常处理：try{...}catch(e){...}捕获运行时错误，throw expr抛出错误，abort(\"msg\")立即终止脚本并报错。外部库：1. 使用 questionnaire:importqlg(path) 导入 .qlg 库文件到 /sdcard/Download/Operit/questionnaire/QLangRuntime/library/；2. 在 resultcode 中用 #include <库名>（不含 .qlg 后缀）展开库内容；3. 库文件格式：以 #defNS 命名空间名 开头声明所属命名空间，默认 qlgstd；4. using namespace qlgstd 切换当前代码使用的命名空间（不写则默认 qlgstd）；5. 主代码中不允许使用 #defNS，该指令仅用于外部库文件：6. 库文件可提供 main 函数，主代码只需 #include <库名> 即可运行；7. 示例库文件（保存为 math.qlg 后用 importqlg 导入）：   #defNS qlgstd\n   using namespace qlgstd;\n   int add(int a, int b) { return a + b; }\n   int fact(int n) { if(n<=1) return 1; return n*fact(n-1); }\n   然后在 resultcode 中使用：#include <math>\n   int main(){ printf(\"%d\",add(3,4)); return 0; }\n示例：struct Node{int v;int next;}; int main(){Node* h=0; Node* n=new Node; n->v=1; n->next=h; h=n; printf(\"%d\",h->v); return 0; }",
            "en": "Result script, use <resultcode> sub-XML tag. C++-like, must have int main()+return 0;. Semicolons, no indent. Types: int/bool/string/char, 1D/2D arrays, for/while(nested break continue), if-else(and or not), arithmetic(including modulo), comparisons, string concat, print(comma multi-arg)/cout, ++/--/+=, const, 1e9, comments. QID via param: int main(int qid), access via qid->q1_num(int) and qid->q1_text(string). Mappings: text/textarea→num=-1 text=raw(textarea \n escaped); single→num=1 text=option; multiple→num=count text=options space-separated; rating/likert/nps→num=value text=value; time→num=timestamp(sec) text=HH:MM:SS. Built-in: _gcd, parseInt. Custom functions. PHP-style: $var=value;. STL(cap 1000): stack/queue/vector/pair/priority_queue. cstring: sizeof/size/strlen/strcmp/strcpy. printf(%d/%s/%x/%o/%p). Pointers(&address/*deref). Scope chain(block/function). 256MB address space, overflow error. Total timeout 10s. Struct: struct+new, Node* p=new Node; p->val=1; p->next=0; while(p!=0) p=p->next. null pointer error. Exception: try{...}catch(e){...} catch runtime errors, throw expr, abort(\"msg\") terminate script with error.\nexample: struct Node{int v;int next;}; int main(){Node* h=0; Node* n=new Node; n->v=1; n->next=h; h=n; printf(\"%d\",h->v); return 0; }"
          },
          "type": "string",
          "required": false
        },
