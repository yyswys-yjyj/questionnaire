    ...
    "tools":[
        {
            ...
            "parameters":[
                ...
                },
                {
                    "name":"resultcode",
                    "description":{
                        "zh":"结果脚本(可选)，与result互斥。QLang v3：问卷XML内用<resultcode>子标签包裹代码。类C++弱缩进脚本，必须写int main()入口并return 0;。分号分隔，无需缩进。\n类型：int/int32(±2.1B)、short(±32K)、long(±2.1B)、longlong/int64(±9e15，此外 longlong 是连在一起的不加空格)、unsigned/uint(0~4.29B)、float/double、bool、string、char。整型补码回绕(INT_MAX+1→INT_MIN)。类型转换函数：_short(v)、_int(v)、_long(v)、_longlong(v)、_int64(v)、_uint(v)、_unsigned(v)、_int32(v)。\n基础：一维二维数组、for/while(嵌套break continue)、if-else(含and or not)、四则运算含取模(整型补码回绕/浮点IEEE754)、位运算(& | ^ ~ << >> 优先级同C)、比较、字符串拼接、print(逗号多参)/cout输出、i++/--/+=、const只读变量、1e9、注释。\n三目表达式：a>b ? a : b，支持递归。示例：int gcd(int a,int b){ return b==0 ? a : gcd(b,a%b); }（欧几里得算法）\n标签跳转（锚点，仅同一函数内生效，不重置变量）：setap 标签名; gotoap 标签名; 标签名支持数字/字母/十六进制组合（如 1、A、1A）\n示例：int i=0; setap 1; printf(\"%d\",i); i++; if(i<3) gotoap 1;\n问卷数据通过引用传参：int main(int &qid)，通过 qid.q1_num(int) 和 qid.q1_text(string) 点号访问。各题型映射：text/textarea→num=-1 text=原文(textarea换行符转\n)；single→num=1 text=选项文本；multiple→num=选择数 text=选项空格隔开；rating/likert/nps→num=按钮编号 text=编号；time→num=时间戳秒数 text=HH:MM:SS。\n 解除变量定义：#gc [value]\n内置函数：_gcd parseInt。自定义函数。PHP变量：$var=value。\nSTL容器(容量1000)：stack push/pop/top/size/empty；queue push/pop/front/back/size；vector push_back/pop_back/get/set/size/clear；pair first/second；priority_queue大顶堆 push/pop/top/size/empty。\ncstring：sizeof size strlen strcmp strcpy。printf格式化(%d/%s/%x/%o/%p/%f)。\n引用传参(int &a)、指针(&取地址/*解引用)。链式作用域(块/函数)。地址表空间256MB，访问违例报错。总时限10秒。\n结构体：struct定义，Node* p=new Node; p->val=1; p->next=0; while(p!=0) p=p->next。\n异常处理：try{...}catch(e){...}捕获运行时错误，throw expr抛出错误，abort(\"msg\")立即终止脚本并报错。\n外部库：1. 使用 questionnaire:importqlg(path) 导入 .qlg 库文件到 /sdcard/Download/Operit/questionnaire/QLangRuntime/library/；2. 在 resultcode 中用 #include <库名>（不含 .qlg 后缀）展开库内容；3. 库文件格式：以 #defNS 命名空间名 开头声明所属命名空间，默认 qlgstd；4. using namespace qlgstd 切换当前代码使用的命名空间（不写则默认 qlgstd）；5. 主代码中不允许使用 #defNS，该指令仅用于外部库文件；6. 库文件可提供 main 函数，主代码只需 #include <库名> 即可运行；7. 示例库文件（保存为 math.qlg 后用 importqlg 导入）：\n #defNS qlgstd\n using namespace qlgstd;\n int add(int a, int b) { return a + b; }\n int fact(int n) { if(n<=1) return 1; return n*fact(n-1); }\n 然后在 resultcode 中使用：#include <math>\n int main(){ printf(\"%d\",add(3,4)); return 0; }\n示例：struct Node{int v;int next;}; int main(){Node* h=0; Node* n=new Node; n->v=1; n->next=h; h=n; printf(\"%d\",h->v); return 0; }",
                        "en":"Result script(QLang V3), use <resultcode> sub-XML tag. C++-like, must have int main()+return 0;. Semicolons, no indent.\nTypes: int/int32(±2.1B), short(±32K), long(±2.1B), longlong/int64(±9e15), unsigned/uint(0~4.29B), float/double, bool, string, char. Two's complement wrap (INT_MAX+1→INT_MIN). Type cast: _short(v), _int(v), _long(v), _longlong(v), _int64(v), _uint(v), _unsigned(v), _int32(v).\n1D/2D arrays, for/while(nested break continue), if-else(and or not), arithmetic(2's complement for int, IEEE754 for float), bitwise(& | ^ ~ << >> C precedence), comparisons, string concat, print(comma multi-arg)/cout, ++/--/+=, const, 1e9, comments.\nTernary: a>b ? a : b, supports recursion. e.g. int gcd(int a,int b){ return b==0 ? a : gcd(b,a%b); }\nLabel jump (anchor, function-scoped only, no var reset): setap name; gotoap name; name supports numbers/letters/hex (e.g. 1, A, 1A)\nExample: int i=0; setap 1; printf(\"%d\",i); i++; if(i<3) gotoap 1;\nQID via ref: int main(int &qid), access via qid.q1_num(int) and qid.q1_text(string). Mappings: text/textarea→num=-1 text=raw(textarea \\n escaped); single→num=1 text=option; multiple→num=count text=options space-separated; rating/likert/nps→num=value text=value; time→num=timestamp(sec) text=HH:MM:SS.\nUndefine variable: #gc [value]\nBuilt-in: _gcd, parseInt. Custom functions. PHP-style: $var=value;.\nSTL(cap 1000): stack/queue/vector/pair/priority_queue. cstring: sizeof/size/strlen/strcmp/strcpy. printf(%d/%s/%x/%o/%p/%f).\nRef params(int &a), pointers(&address/*deref). Scope chain(block/function). 256MB address space, access violation error. Total timeout 10s.\nStruct: struct+new, Node* p=new Node; p->val=1; p->next=0; while(p!=0) p=p->next.\nException: try{...}catch(e){...} catch runtime errors, throw expr, abort(\"msg\") terminate script with error.\nExternal lib: 1. questionnaire:importqlg(path) to import .qlg lib into /sdcard/Download/Operit/questionnaire/QLangRuntime/library/; 2. #include <libname> (without .qlg suffix) in resultcode; 3. Library format: starts with #defNS namespace_name (default qlgstd); 4. using namespace qlgstd to switch namespace; 5. #defNS is for library files only, not allowed in main code; 6. Library can provide main function; 7. Example library (save as math.qlg then importqlg):\n #defNS qlgstd\n using namespace qlgstd;\n int add(int a, int b) { return a + b; }\n int fact(int n) { if(n<=1) return 1; return n*fact(n-1); }\n Then in resultcode: #include <math>\n int main(){ printf(\"%d\",add(3,4)); return 0; }\nexample: struct Node{int v;int next;}; int main(){Node* h=0; Node* n=new Node; n->v=1; n->next=h; h=n; printf(\"%d\",h->v); return 0; }"
                    },
                    "type":"string",
                    "required":false
                },
                {
                    "name":"qinitcode",
                    "description":{
                        "zh":"QinitCode 问卷预处理脚本(可选)。QinitCode 是完全自研的 DSL，脱离 QLang 语法，专用于问卷初始化和自定义题型注册。\n基础：\n @Qinit{ ... } — 问卷初始化块，自动执行。\n   $a = 123; @QinitSet(\"q1\", $a); — 设置初始值。\n   $v = @SearchQuestionID(\"题名\", \"题型\"); — 搜索 qid。\n   @RunFunc(name); — 调用已注册函数。\n @Regfunc<ret>Param:$a,$b&{ ... } — 注册函数，& 引用传参。\n   @SetCallBackName(\"name\"); — 回调名（重复报错）。\n   @CallBackReturn(val); — 返回结果。\n   @RunQLangCode(Send:{int a}=>1 &Return:$e &CollectFunc:main){ QLang代码 } — 桥接QLang。\n   @GetEventInfo(source, field); — 查询信息。\n @RegQuestion{ ... } — 注册自定义题型。\n   @SetQName(\"name\"); — 题型名（必填）。\n   @ComponentSet{ @RegAnserTool({...},\"字段\",*&){ ... } } — 回答工具。\n     参数1: apiSpec(JSON对象,含AnserComponent指定渲染组件)\n     参数2: 字段名(如\"option\")\n     参数3: 接收格式(*&通配或RECEIVER格式)\n     block内: @AnserEventReturn(val) — 返回处理后的答案\n     可用AnserComponent: singleChoice(单选) multipleChoice(多选) textField(文本) textArea(多行文本) rating(评分)\n     示例: @RegAnserTool({\"AnserComponent\":\"singleChoice\"},\"option\",*&){ $r = @StringFormat(\"已选: \" + $ans); @AnserEventReturn($r); }\n     *& — 通配接收任意值。\n     RECEIVER:[\"*\",...]& — 接收数组。\n运算符：+ - * / % == != < > <= >= && || ! ?: $a.b\n控制：if/else for(1:10) for(init;cond;inc) while break continue\n工具：@len @typeOf @toString @toInt @toBool @StringFormat @join @throw @log",
                        "en":"QinitCode questionnaire preprocessing script (optional). A self-developed DSL for questionnaire init and custom question type registration.\nBasics:\n @Qinit{ ... } — auto-executed init block.\n @Regfunc<ret>Param:$a,$b&{ ... } — register function with ref params.\n @RunFunc(name); — invoke registered function.\n @RunQLangCode(Send:{int a}=>1 &Return:$e &CollectFunc:main){ QLang code } — bridge to QLang.\n @RegQuestion{ ... } — register custom question type.\n   @SetQName(\"name\"); — required.\n   @ComponentDef({...}); — component properties.\n   @ComponentSet{ @RegAnserTool({...},\"field\",*&){ ... } } — answer handler.\n     Param1: apiSpec(JSON, AnserComponent specifies renderer)\n     Param2: field name(e.g. \"option\")\n     Param3: receiver format(*& or RECEIVER)\n     In block: @AnserEventReturn(val) — return processed answer\n     Available AnserComponent: singleChoice multipleChoice textField textArea rating\n     Example: @RegAnserTool({\"AnserComponent\":\"singleChoice\"},\"option\",*&){ $r = @StringFormat(\"selected: \" + $ans); @AnserEventReturn($r); }\n     *& — any single value.\n     RECEIVER:[\"*\",...]& — array of values.\nOperators: + - * / % == != < > <= >= && || ! ?: $a.b\nControl: if/else for(1:10) for(init;cond;inc) while break continue\nUtils: @len @typeOf @toString @toInt @toBool @StringFormat @join @throw @log"
                    },
                    "type":"string",
                    "required":false
                },
...
var askPackage = (function () {
    ...
    async function ask(params) {
        ...
        // QinitCode 预处理：运行 qinitcode 脚本，注册自定义题型
        var _qcRunTypes = {};
        if (params.qinitcode && typeof params.qinitcode === 'string' && params.qinitcode.trim().length > 0) {
            try {
                var _qcMod = require('./qinitcode/index.js');
                if (_qcMod && _qcMod.compileAndRun) {
                    var _qsMap = {};
                    for (var _qci3 = 0; _qci3 < questions.length; _qci3++) {
                        if (questions[_qci3].id) {
                            _qsMap[questions[_qci3].id] = { title: questions[_qci3].question, type: questions[_qci3].type };
                        }
                    }
                    var _qcR = _qcMod.compileAndRun(params.qinitcode, { questionMap: _qsMap });
                    if (_qcR && _qcR.questions) {
                        for (var _qtk in _qcR.questions) {
                            _qcRunTypes[_qtk] = true;
                        }
                    }
                }
            } catch (_qcE) {
        ...
        // 合并 QinitCode 注册的自定义题型
        for (var _qcTk in _qcRunTypes) { validTypes[_qcTk] = true; }
        ...
        for (var vi = 0; vi < questions.length; vi++) {
            ...
            if (!validTypes[vq.type]) {
                var _qcTypesList = "section/single/multiple/text/textarea/rating/likert/nps";
                for (var _qcTk2 in _qcRunTypes) _qcTypesList += "/" + _qcTk2;
                addError("第" + (vi + 1) + "题 type 不合法: " + vq.type + "，合法值: " + _qcTypesList);
            }
        ...
        if (params.roll_spec === true)
            xmlObj.roll_spec = true;
        if (params.qinitcode && typeof params.qinitcode === 'string' && params.qinitcode.trim().length > 0) {
            xmlObj.qinitcode = params.qinitcode.replace(/\n/g, '\\n').replace(/\r/g, '');
        }
