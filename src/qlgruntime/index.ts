...
function executeAst(ast, answers, otherInputs, qs) {
    ...
    // 放在 globalEnv 初始化之后，确保 __structs__ 已经存在
    // 只有有题目时才注册 qid 变量
    if (_qidAddr !== 0) {
    ...
    // 如果已有 __qid__ 定义，同步到 globalEnv.__structs__
    ...
    // 同步到 QLangExecutor 和 QLangBuiltins
...
export async function executeQLang(resultCodeStr, answers, otherInputs, qs) {
    try {
        ...
        // 预处理 #include 指令：替换为库文件内容
        var _incRegex = /#include\s*<([^>]+)>/g;
        var _incMatch;
        while ((_incMatch = _incRegex.exec(code)) !== null) {
            var _libName = _incMatch[1];
            var _libDir = '/sdcard/Download/Operit/questionnaire/QLangRuntime/library';
            try {
                var _libRes = await Tools.Files.read({ path: _libDir + '/' + _libName + '.qlg' });
                var _libRaw = (typeof _libRes === 'object' && _libRes && _libRes.content) ? String(_libRes.content) : String(_libRes || '');
                _libRaw = stripLineNumbers(_libRaw);
                // 去掉 #defNS 和 using namespace 行
                _libRaw = _libRaw.replace(/^#defNS\s+\w+;?\s*$/gm, '').replace(/^using\s+namespace\s+\w+;?\s*$/gm, '').trim();
                code = code.substring(0, _incMatch.index) + '\n' + _libRaw + '\n' + code.substring(_incMatch.index + _incMatch[0].length);
                _incRegex.lastIndex = _incMatch.index; // 重新扫描
            } catch (_ie) {
                throw new Error('脚本错误: [库: ' + _libName + '.qlg] 读取失败: ' + (_ie.message || String(_ie)));
            }
        }
        var ast = parse(code, {});
